...
public class A2PdfHarness {

    @Test
    void genererEchantillon() throws Exception {
        RelevNoteA2 r = new RelevNoteA2();
        ...
        r.setNotesPremierGroupe(List.of(
                note(MatieresA2.FRANCAIS_ECRIT.getCode(), 11, 33),
                note(MatieresA2.FRANCAIS_ORAL.getCode(), 11, 11),
                note(MatieresA2.PHILOSOPHIE.getCode(), 11, 44),
                note(MatieresA2.LATIN_GREC_1.getCode(), 11, 33),
                note(MatieresA2.HIST_GEO.getCode(), 11, 33),
                note(MatieresA2.LANGUE_VIVANTE.getCode(), 11, 22)
        ));
        r.setTotalPremierGroupe(176);
        r.setReportPremierTotal(176);

        r.setNotesDeuxiemeGroupe(List.of(
                note(MatieresA2.GREC_LATIN_2.getCode(), 12, 24),
                note(MatieresA2.MATHEMATIQUES.getCode(), 13, 26)
        ));
