    ...
    public SecurityFilterChain jwtFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/api/v1/parametrage/**").hasAuthority(Role.ADMIN.name())
                        .requestMatchers("/api/v1/enrollment-candidats/**").hasAnyAuthority(Role.ADMIN.name(), Role.CHEF_ETABLISSEMENT.name(), Role.AGENT_DE_SAISIE.name(), Role.SCOLARITE.name(), Role.RECEPTIONNISTE.name(), Role.AUTORISATION_RECEPTION.name(), Role.VIGNETTES_COUPONS.name())
                        .requestMatchers("/api/v1/security/**").hasAnyAuthority(Role.ADMIN.name(), Role.CHEF_ETABLISSEMENT.name(), Role.AGENT_DE_SAISIE.name(), Role.SCOLARITE.name(), Role.RECEPTIONNISTE.name(), Role.AUTORISATION_RECEPTION.name(), Role.VIGNETTES_COUPONS.name())
                        .requestMatchers("/api/v1/files/**").hasAnyAuthority(Role.CHEF_ETABLISSEMENT.name(), Role.AGENT_DE_SAISIE.name(), Role.SCOLARITE.name(), Role.ADMIN.name(), Role.AUTORISATION_RECEPTION.name(), Role.VIGNETTES_COUPONS.name())
                        .requestMatchers("/api/v1/validation-candidats/**").hasAnyAuthority(Role.AUTORISATION_RECEPTION.name(), Role.SCOLARITE.name(), Role.VIGNETTES_COUPONS.name(), Role.AGENT_DE_SAISIE.name(), Role.ADMIN.name(), Role.RECEPTIONNISTE.name())
                        .requestMatchers("/api/v1/payment-FAEB3/**").hasAnyAuthority(Role.CHEF_ETABLISSEMENT.name(), Role.AGENT_DE_SAISIE.name(), Role.ADMIN.name())
