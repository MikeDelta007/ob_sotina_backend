package com.officedubac.project.modules.T2.service;
...
import com.officedubac.project.modules.T2.model.Enums.DecisionJury;
import com.officedubac.project.modules.T2.model.Enums.Mention;
import com.officedubac.project.modules.T2.repository.RelevNoteT2Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série T2.
 */
@Service
public class RelevNoteT2Service {

    private final RelevNoteT2Repository repository;

    public RelevNoteT2Service(RelevNoteT2Repository repository) {
        this.repository = repository;
    }

    public RelevNoteT2 creer(RelevNoteT2SaisieRequest request) {
        RelevNoteT2 releve = new RelevNoteT2();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteT2 mettreAJour(String id, RelevNoteT2SaisieRequest request) {
        RelevNoteT2 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteT2 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteT2Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteT2> page;
    ...
    private RelevNoteT2Resume versResume(RelevNoteT2 r) {
    ...
    private void appliquer(RelevNoteT2 releve, RelevNoteT2SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresT2.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresT2.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteT2SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteT2SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresT2.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresT2.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresT2.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresT2.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresT2.BAREME_TOTAL_DEFINITIF));
