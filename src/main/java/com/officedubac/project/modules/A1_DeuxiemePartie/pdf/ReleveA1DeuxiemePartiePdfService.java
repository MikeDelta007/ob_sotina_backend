...
import com.officedubac.project.modules.A1_DeuxiemePartie.model.Candidat;
...
import static com.officedubac.project.modules.A1_DeuxiemePartie.pdf.ReleveA1DeuxiemePartieCoordinates.*;
 ...
 * coordonnées mesurées dans {@link ReleveA1DeuxiemePartieCoordinates}.
 *
 * Même principe que RelevNoteA1PdfService (module A1 1ère partie) : le fond
 * du PDF généré est le scan officiel, donc la mise en page (bordures,
 * libellés, barèmes imprimés) reste identique à 100 %. Seule la position
 * des valeurs saisies dépend du calage mesuré — à revalider si un autre
 * scan est utilisé comme template.
 */
@Service
public class ReleveA1DeuxiemePartiePdfService {
    ...
    private void ecrireEpreuvesEcrites(PdfContentByte cb, BaseFont font, ReleveA1DeuxiemePartie r) {
        ...
        ligneNote(cb, font, parCode.get(MatieresA1DeuxiemePartie.PHILOSOPHIE.getCode()),    EC_NOTE_X, EC_POINTS_X, EC_PHILO_Y);
        ligneNote(cb, font, parCode.get(MatieresA1DeuxiemePartie.LATIN_ARABE.getCode()),    EC_NOTE_X, EC_POINTS_X, EC_LATAR_Y);
        ligneNote(cb, font, parCode.get(MatieresA1DeuxiemePartie.GREC.getCode()),           EC_NOTE_X, EC_POINTS_X, EC_GREC_Y);
        ligneNote(cb, font, parCode.get(MatieresA1DeuxiemePartie.LANGUE_VIVANTE.getCode()), EC_NOTE_X, EC_POINTS_X, EC_LV_Y);
    ...
    private void ecrireEpreuvesOrales(PdfContentByte cb, BaseFont font, ReleveA1DeuxiemePartie r) {
        ...
        ligneNote(cb, font, parCode.get(MatieresA1DeuxiemePartie.LATIN_GREC_ARABE_ORAL.getCode()), OR_NOTE_X, OR_POINTS_X, OR_LATGRAR_Y);
        ligneNote(cb, font, parCode.get(MatieresA1DeuxiemePartie.HIST_GEO.getCode()),               OR_NOTE_X, OR_POINTS_X, OR_HISTGEO_Y);
        ligneNote(cb, font, parCode.get(MatieresA1DeuxiemePartie.MATHEMATIQUES.getCode()),          OR_NOTE_X, OR_POINTS_X, OR_MATHS_Y);
    ...
    private void ecrireTotauxEtDecision(PdfContentByte cb, BaseFont font, ReleveA1DeuxiemePartie r) {
