package com.officedubac.project.modules.F7.pdf;
...
import static com.officedubac.project.modules.F7.pdf.RelevNoteF7Coordinates.*;

/**
 * Génère le PDF du relevé F7 en superposant les données saisies sur le
 * gabarit officiel (fond de page), aux coordonnées mesurées dans {@link RelevNoteF7Coordinates}.
 */
@Service
public class RelevNoteF7PdfService {

    private static final String TEMPLATE_PATH = "templates/releve-F7-template.pdf";
    ...
    public byte[] genererPdf(RelevNoteF7 releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé F7 en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF F7", ex);
            }
        }
    }
    ...
    private void ecrireSession(PdfContentByte cb, RelevNoteF7 r) {
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, RelevNoteF7 r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, RelevNoteF7 r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteF7 r) {
        ...
        List<Matiere> matieres = MatieresF7.PREMIER_GROUPE;
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteF7 r) {
        ...
        Map<String, NoteEpreuve> parCode = r.getNotesDeuxiemeGroupe().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresF7.DEUXIEME_GROUPE;
        for (int i = 0; i < matieres.size(); i++) {
            ligneNote(cb, font, parCode.get(matieres.get(i).getCode()), G2_NOTE_CENTER_X, G2_POINTS_CENTER_X, G2_ROW_Y[i]);
        }
    }

    private void ecrireEpreuvesControle(PdfContentByte cb, BaseFont font, RelevNoteF7 r) {
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteF7 r) {
    ...
    private void ecrireTotaux(PdfContentByte cb, BaseFont font, RelevNoteF7 r) {
        texteCentre(cb, font, TAILLE_TOTAUX, G2_POINTS_CENTER_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, RelevNoteF7 r) {
    ...
    private void ecrirePiedDePage(PdfContentByte cb, BaseFont font, RelevNoteF7 r, float lieuX, float jourMoisX, float annee2X, float y) {
