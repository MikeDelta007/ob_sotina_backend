package com.officedubac.project.modules.F7.service;
...
import com.officedubac.project.modules.F7.repository.RelevNoteF7Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série F7.
 */
@Service
public class RelevNoteF7Service {

    private final RelevNoteF7Repository repository;

    public RelevNoteF7Service(RelevNoteF7Repository repository) {
        this.repository = repository;
    }

    public RelevNoteF7 creer(RelevNoteF7SaisieRequest request) {
        RelevNoteF7 releve = new RelevNoteF7();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteF7 mettreAJour(String id, RelevNoteF7SaisieRequest request) {
        RelevNoteF7 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteF7 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteF7Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteF7> page;
    ...
    private RelevNoteF7Resume versResume(RelevNoteF7 r) {
    ...
    private void appliquer(RelevNoteF7 releve, RelevNoteF7SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresF7.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresF7.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteF7SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteF7SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresF7.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresF7.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresF7.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresF7.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresF7.BAREME_TOTAL_DEFINITIF));
