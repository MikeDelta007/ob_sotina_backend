package com.officedubac.project.modules.L1A.service;
...
import com.officedubac.project.modules.L1A.repository.RelevNoteL1ARepository;
 ...
 * selon les règles imprimées sur le formulaire officiel série L1A.
 */
@Service
public class RelevNoteL1AService {

    private final RelevNoteL1ARepository repository;

    public RelevNoteL1AService(RelevNoteL1ARepository repository) {
        this.repository = repository;
    }

    public RelevNoteL1A creer(RelevNoteL1ASaisieRequest request) {
        RelevNoteL1A releve = new RelevNoteL1A();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteL1A mettreAJour(String id, RelevNoteL1ASaisieRequest request) {
        RelevNoteL1A releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteL1A obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteL1AResume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteL1A> page;
    ...
    private RelevNoteL1AResume versResume(RelevNoteL1A r) {
    ...
    private void appliquer(RelevNoteL1A releve, RelevNoteL1ASaisieRequest request) {
        ...
        for (Matiere matiere : MatieresL1A.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresL1A.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteL1ASaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteL1ASaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresL1A.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresL1A.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresL1A.BAREME_TOTAL_DEFINITIF);
