...
/**
 * Génère le PDF du relevé A1 en superposant les données saisies sur le
 * nouveau gabarit vectoriel (fond de page), aux coordonnées mesurées dans
 * {@link RelevNoteA1Coordinates}.
 *
 * Nouveautés par rapport à la version précédente :
 * - Police Verdana embarquée (voir {@link #chargerPolice()} et le README
 *   livré avec ce module — le fichier Verdana.ttf doit être fourni côté
 *   projet, il ne peut pas être redistribué ici pour des raisons de licence).
 * - Tailles de police adaptées au contexte de chaque champ (grille de notes
 *   en 7pt, identité en 9pt, totaux en 8pt...).
 * - Notes/points centrés dans leur cellule (et non plus alignés à gauche).
 * - Cases à cocher réelles pour la décision du jury (au lieu d'une simple
 *   petite croix à côté du texte).
 * - SESSION (1) : l'option NON choisie (NORMALE ou DE REMPLACEMENT) est
 *   barrée d'un trait, comme le ferait un jury à la main.
 * - Date "Fait à ... le ... 19..." : "le" reçoit le jour/mois (dd/MM), et
 *   les deux derniers chiffres de l'année sont ajoutés après le "19" déjà
 *   imprimé.
 */
@Service
public class RelevNoteA1PdfService {
    ...
    private void ecrireSession(PdfContentByte cb, RelevNoteA1 r) {
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteA1 r) {
        ...
        ligneNote(cb, font, parCode.get(MatieresA1.FRANCAIS_ECRIT.getCode()), G1_FR_ECRIT_Y);
        ligneNote(cb, font, parCode.get(MatieresA1.FRANCAIS_ORAL.getCode()),  G1_FR_ORAL_Y);
        ligneNote(cb, font, parCode.get(MatieresA1.PHILOSOPHIE.getCode()),    G1_PHILO_Y);
        ligneNote(cb, font, parCode.get(MatieresA1.LATIN_GREC_1.getCode()),   G1_LATGREC_Y);
        ligneNote(cb, font, parCode.get(MatieresA1.HIST_GEO.getCode()),       G1_HISTGEO_Y);
        ligneNote(cb, font, parCode.get(MatieresA1.LANGUE_VIVANTE.getCode()), G1_LV_Y);
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteA1 r) {
        ...
        ligneNote2(cb, font, parCode.get(MatieresA1.GREC_LATIN_2.getCode()),  G2_GRECLATIN_Y);
        ligneNote2(cb, font, parCode.get(MatieresA1.MATHEMATIQUES.getCode()), G2_MATHS_Y);
    }
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteA1 r) {
