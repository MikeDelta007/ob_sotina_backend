package com.officedubac.project.modules.G1.service;
...
import com.officedubac.project.modules.G1.model.*;
import com.officedubac.project.modules.G1.model.Enums.DecisionJury;
import com.officedubac.project.modules.G1.model.Enums.Mention;
import com.officedubac.project.modules.G1.repository.RelevNoteG1Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série G1.
 */
@Service
public class RelevNoteG1Service {

    private final RelevNoteG1Repository repository;

    public RelevNoteG1Service(RelevNoteG1Repository repository) {
        this.repository = repository;
    }

    public RelevNoteG1 creer(RelevNoteG1SaisieRequest request) {
        RelevNoteG1 releve = new RelevNoteG1();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteG1 mettreAJour(String id, RelevNoteG1SaisieRequest request) {
        RelevNoteG1 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteG1 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteG1Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteG1> page;
    ...
    private RelevNoteG1Resume versResume(RelevNoteG1 r) {
    ...
    private void appliquer(RelevNoteG1 releve, RelevNoteG1SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresG1.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresG1.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteG1SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteG1SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresG1.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresG1.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresG1.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresG1.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresG1.BAREME_TOTAL_DEFINITIF));
