...
import static com.officedubac.project.modules.C_Deuxieme_Partie.pdf.ReleveCDeuxiemePartieCoordinates.*;

/**
 * Génère le PDF du relevé C 2ème Partie en superposant les données saisies
 * sur une copie du formulaire officiel scanné (fond de page), aux
 * coordonnées mesurées dans {@link ReleveCDeuxiemePartieCoordinates}.
 */
@Service
public class ReleveCDeuxiemePartiePdfService {

    private static final String TEMPLATE_PATH = "templates/releve-C-2emePartie-template.pdf";
    private static final DateTimeFormatter DATE_FR = DateTimeFormatter.ofPattern("dd/MM/yyyy");

    public byte[] genererPdf(ReleveCDeuxiemePartie releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé C 2ème Partie en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, ReleveCDeuxiemePartie r) {
        texte(cb, font, 9, JURY_NUMERO_X, JURY_NUMERO_Y, r.getJuryNumero());
    }

    private void ecrireCandidat(PdfContentByte cb, BaseFont font, ReleveCDeuxiemePartie r) {
    ...
    private void ecrireEpreuvesEcrites(PdfContentByte cb, BaseFont font, ReleveCDeuxiemePartie r) {
        Map<String, NoteEpreuve> parCode = r.getNotesEcrites().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n));

        ligneNote(cb, font, parCode.get(MatieresCDeuxiemePartie.PHILOSOPHIE.getCode()),         EC_NOTE_X, EC_POINTS_X, EC_PHILO_Y);
        ligneNote(cb, font, parCode.get(MatieresCDeuxiemePartie.MATHEMATIQUES_ECRIT.getCode()), EC_NOTE_X, EC_POINTS_X, EC_MATH_Y);
        ligneNote(cb, font, parCode.get(MatieresCDeuxiemePartie.SCIENCES_PHYSIQUES.getCode()),  EC_NOTE_X, EC_POINTS_X, EC_SCPHY_Y);
        ligneNote(cb, font, parCode.get(MatieresCDeuxiemePartie.SCIENCES_NATURELLES.getCode()), EC_NOTE_X, EC_POINTS_X, EC_SCNAT_Y);

        texte(cb, font, 9, EC_TOTAL_POINTS_X, EC_TOTAL_Y, str(r.getTotalEcrit()));
    }

    private void ecrireEpreuvesOrales(PdfContentByte cb, BaseFont font, ReleveCDeuxiemePartie r) {
        Map<String, NoteEpreuve> parCode = r.getNotesOrales().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n));

        ligneNote(cb, font, parCode.get(MatieresCDeuxiemePartie.LV1.getCode()),               OR_NOTE_X, OR_POINTS_X, OR_LV1_Y);
        ligneNote(cb, font, parCode.get(MatieresCDeuxiemePartie.HIST_GEO.getCode()),           OR_NOTE_X, OR_POINTS_X, OR_HISTGEO_Y);
        ligneNote(cb, font, parCode.get(MatieresCDeuxiemePartie.MATHEMATIQUES_ORAL.getCode()), OR_NOTE_X, OR_POINTS_X, OR_MATH_Y);
    ...
    private void ecrireEpsEtTotal(PdfContentByte cb, BaseFont font, ReleveCDeuxiemePartie r) {
    ...
    private void ecrireDecision(PdfContentByte cb, BaseFont font, ReleveCDeuxiemePartie r) {
