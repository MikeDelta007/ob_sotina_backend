package com.officedubac.project.modules.S4.pdf;
...
import com.officedubac.project.modules.S4.model.Enums.*;
...
import static com.officedubac.project.modules.S4.pdf.RelevNoteS4Coordinates.*;

/**
 * Génère le PDF du relevé S4 en superposant les données saisies sur le
 * gabarit officiel (fond de page), aux coordonnées mesurées dans {@link RelevNoteS4Coordinates}.
 */
@Service
public class RelevNoteS4PdfService {

    private static final String TEMPLATE_PATH = "templates/releve-S4-template.pdf";
    ...
    public byte[] genererPdf(RelevNoteS4 releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé S4 en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF S4", ex);
            }
        }
    }
    ...
    private void ecrireSession(PdfContentByte cb, RelevNoteS4 r) {
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, RelevNoteS4 r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, RelevNoteS4 r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteS4 r) {
        ...
        List<Matiere> matieres = MatieresS4.PREMIER_GROUPE;
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteS4 r) {
        ...
        Map<String, NoteEpreuve> parCode = r.getNotesDeuxiemeGroupe().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresS4.DEUXIEME_GROUPE;
        for (int i = 0; i < matieres.size(); i++) {
            ligneNote(cb, font, parCode.get(matieres.get(i).getCode()), G2_NOTE_CENTER_X, G2_POINTS_CENTER_X, G2_ROW_Y[i]);
        }
    }

    private void ecrireEpreuvesControle(PdfContentByte cb, BaseFont font, RelevNoteS4 r) {
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteS4 r) {
    ...
    private void ecrireTotaux(PdfContentByte cb, BaseFont font, RelevNoteS4 r) {
        texteCentre(cb, font, TAILLE_TOTAUX, G2_POINTS_CENTER_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, RelevNoteS4 r) {
    ...
    private void ecrirePiedDePage(PdfContentByte cb, BaseFont font, RelevNoteS4 r, float lieuX, float jourMoisX, float annee2X, float y) {
