package com.officedubac.project.modules.S4.service;
...
import com.officedubac.project.modules.S4.model.*;
import com.officedubac.project.modules.S4.model.Enums.DecisionJury;
import com.officedubac.project.modules.S4.model.Enums.Mention;
import com.officedubac.project.modules.S4.repository.RelevNoteS4Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série S4.
 */
@Service
public class RelevNoteS4Service {

    private final RelevNoteS4Repository repository;

    public RelevNoteS4Service(RelevNoteS4Repository repository) {
        this.repository = repository;
    }

    public RelevNoteS4 creer(RelevNoteS4SaisieRequest request) {
        RelevNoteS4 releve = new RelevNoteS4();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteS4 mettreAJour(String id, RelevNoteS4SaisieRequest request) {
        RelevNoteS4 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteS4 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteS4Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteS4> page;
    ...
    private RelevNoteS4Resume versResume(RelevNoteS4 r) {
    ...
    private void appliquer(RelevNoteS4 releve, RelevNoteS4SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresS4.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresS4.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteS4SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteS4SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresS4.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresS4.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresS4.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresS4.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresS4.BAREME_TOTAL_DEFINITIF));
