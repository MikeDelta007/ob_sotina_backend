package com.officedubac.project.modules.f1deuxiemepartie.pdf;

import com.lowagie.text.pdf.BaseFont;
import com.lowagie.text.pdf.PdfContentByte;
import com.lowagie.text.pdf.PdfReader;
import com.lowagie.text.pdf.PdfStamper;
import com.officedubac.project.modules.f1deuxiemepartie.model.*;
...
import static com.officedubac.project.modules.f1deuxiemepartie.pdf.ReleveF1DeuxiemePartieCoordinates.*;

/**
 * Génère le PDF du relevé F1-2emePartie en superposant les données saisies sur le
...
public class ReleveF1DeuxiemePartiePdfService {
    ...
    public byte[] genererPdf(ReleveF1DeuxiemePartie releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé F1-2emePartie en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF F1-2emePartie", ex);
            }
        }
    }
    ...
    private void ecrireEpreuvesEcrites(PdfContentByte cb, BaseFont font, ReleveF1DeuxiemePartie r) {
        ...
        List<Matiere> matieres = MatieresF1DeuxiemePartie.EPREUVES_ECRITES;
    ...
    private void ecrireEpreuvesOrales(PdfContentByte cb, BaseFont font, ReleveF1DeuxiemePartie r) {
        Map<String, NoteEpreuve> parCode = r.getNotesOrales().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresF1DeuxiemePartie.EPREUVES_ORALES;
        for (int i = 0; i < matieres.size(); i++) {
    ...
    private void ecrireTotalGeneral(PdfContentByte cb, BaseFont font, ReleveF1DeuxiemePartie r) {
