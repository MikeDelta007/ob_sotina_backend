...
import static com.officedubac.project.modules.a2deuxiemepartie.pdf.ReleveA2DeuxiemePartieCoordinates.*;
...
public class ReleveA2DeuxiemePartiePdfService {
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF A2 2ème Partie", ex);
            }
        }
    }
    ...
    private void ecrireEpreuvesEcrites(PdfContentByte cb, BaseFont font, ReleveA2DeuxiemePartie r) {
        ...
        ligneNote(cb, font, parCode.get(MatieresA2DeuxiemePartie.PHILOSOPHIE.getCode()), EC_NOTE_CENTER_X, EC_POINTS_CENTER_X, EC_PHILO_Y);
        ligneNote(cb, font, parCode.get(MatieresA2DeuxiemePartie.LV1_ECRIT.getCode()),   EC_NOTE_CENTER_X, EC_POINTS_CENTER_X, EC_LV1_Y);
        ligneNote(cb, font, parCode.get(MatieresA2DeuxiemePartie.HIST_GEO.getCode()),    EC_NOTE_CENTER_X, EC_POINTS_CENTER_X, EC_HISTGEO_Y);
        ligneNote(cb, font, parCode.get(MatieresA2DeuxiemePartie.LATIN_ARABE.getCode()), EC_NOTE_CENTER_X, EC_POINTS_CENTER_X, EC_LATAR_Y);
    ...
    private void ecrireEpreuvesOrales(PdfContentByte cb, BaseFont font, ReleveA2DeuxiemePartie r) {
        ...
        ligneNote(cb, font, parCode.get(MatieresA2DeuxiemePartie.LV1_ORAL.getCode()),      OR_NOTE_CENTER_X, OR_POINTS_CENTER_X, OR_LV1_Y);
        ligneNote(cb, font, parCode.get(MatieresA2DeuxiemePartie.LV2_ORAL.getCode()),      OR_NOTE_CENTER_X, OR_POINTS_CENTER_X, OR_LV2_Y);
        ligneNote(cb, font, parCode.get(MatieresA2DeuxiemePartie.MATHEMATIQUES.getCode()), OR_NOTE_CENTER_X, OR_POINTS_CENTER_X, OR_MATH_Y);
    ...
    private void ecrireTotalGeneral(PdfContentByte cb, BaseFont font, ReleveA2DeuxiemePartie r) {
        texte(cb, font, TAILLE_TOTAL_GEN, TOTAL_GENERAL_X, TOTAL_GENERAL_Y, str(r.getTotalGeneral()));
    }

    private void ecrireDecision(PdfContentByte cb, ReleveA2DeuxiemePartie r) {
    ...
    private void ecrirePiedDePage(PdfContentByte cb, BaseFont font, ReleveA2DeuxiemePartie r) {
        if (r.getDateDeliberation() == null) return;
        texte(cb, font, TAILLE_PIED_PAGE, LIEU_DELIBERATION_X, LIEU_DELIBERATION_Y, r.getLieuDeliberation());
        texte(cb, font, TAILLE_PIED_PAGE, JOUR_MOIS_X, JOUR_MOIS_Y, r.getDateDeliberation().format(DATE_JOUR_MOIS));
        texte(cb, font, TAILLE_PIED_PAGE, ANNEE_2_CHIFFRES_X, ANNEE_2_CHIFFRES_Y, r.getDateDeliberation().format(DATE_ANNEE_2_CHIFFRES));
    }
