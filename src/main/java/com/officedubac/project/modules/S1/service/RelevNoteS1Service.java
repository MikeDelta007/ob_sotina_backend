package com.officedubac.project.modules.S1.service;
...
import com.officedubac.project.modules.S1.model.Enums.DecisionJury;
import com.officedubac.project.modules.S1.model.Enums.Mention;
import com.officedubac.project.modules.S1.repository.RelevNoteS1Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série S1.
 */
@Service
public class RelevNoteS1Service {

    private final RelevNoteS1Repository repository;

    public RelevNoteS1Service(RelevNoteS1Repository repository) {
        this.repository = repository;
    }

    public RelevNoteS1 creer(RelevNoteS1SaisieRequest request) {
        RelevNoteS1 releve = new RelevNoteS1();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteS1 mettreAJour(String id, RelevNoteS1SaisieRequest request) {
        RelevNoteS1 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteS1 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteS1Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteS1> page;
    ...
    private RelevNoteS1Resume versResume(RelevNoteS1 r) {
    ...
    private void appliquer(RelevNoteS1 releve, RelevNoteS1SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresS1.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresS1.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteS1SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteS1SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresS1.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresS1.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresS1.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresS1.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresS1.BAREME_TOTAL_DEFINITIF));
