...
import com.officedubac.project.modules.A3.repository.RelevNoteA3Repository;
 ...
 */
@Service
public class RelevNoteA3Service {

    private final RelevNoteA3Repository repository;

    public RelevNoteA3Service(RelevNoteA3Repository repository) {
        this.repository = repository;
    }

    public RelevNoteA3 creer(RelevNoteA3SaisieRequest request) {
        RelevNoteA3 releve = new RelevNoteA3();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteA3 mettreAJour(String id, RelevNoteA3SaisieRequest request) {
        RelevNoteA3 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteA3 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteA3Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteA3> page;
    ...
    private RelevNoteA3Resume versResume(RelevNoteA3 r) {
    ...
    private void appliquer(RelevNoteA3 releve, RelevNoteA3SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresA3.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteA3SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteA3SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
