...
import static com.officedubac.project.modules.A3.pdf.ReleveA3Coordinates.*;

/**
 * Génère le PDF du relevé A3 en superposant les données saisies sur une
 * copie du formulaire officiel scanné (fond de page), aux coordonnées
 * mesurées dans {@link ReleveA3Coordinates}. Même principe que les modules
 * A1 / A2 : le fond du PDF généré EST le scan officiel.
 */
@Service
public class ReleveA3PdfService {

    private static final String TEMPLATE_PATH = "templates/releve-A3-template.pdf";
    private static final DateTimeFormatter DATE_FR = DateTimeFormatter.ofPattern("dd/MM/yyyy");

    public byte[] genererPdf(ReleveA3 releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé A3 en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, ReleveA3 r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, ReleveA3 r) {
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, ReleveA3 r) {
        var parCode = r.getNotesPremierGroupe().stream()
                .collect(java.util.stream.Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n));

        ligneNote(cb, font, parCode.get(MatieresA3.FRANCAIS_ECRIT.getCode()), G1_FR_ECRIT_Y);
        ligneNote(cb, font, parCode.get(MatieresA3.FRANCAIS_ORAL.getCode()),  G1_FR_ORAL_Y);
        ligneNote(cb, font, parCode.get(MatieresA3.PHILOSOPHIE.getCode()),    G1_PHILO_Y);
        ligneNote(cb, font, parCode.get(MatieresA3.HIST_GEO.getCode()),      G1_HISTGEO_Y);
        ligneNote(cb, font, parCode.get(MatieresA3.LV1_ECRIT.getCode()),     G1_LV1_ECRIT_Y);
        ligneNote(cb, font, parCode.get(MatieresA3.MATHEMATIQUES.getCode()), G1_MATH_Y);
        ligneNote(cb, font, parCode.get(MatieresA3.LV2.getCode()),           G1_LV2_Y);
        ligneNote(cb, font, parCode.get(MatieresA3.LV1_ORAL.getCode()),      G1_LV1_ORAL_Y);
    ...
    private void ecrireEpreuvesDeControle(PdfContentByte cb, BaseFont font, ReleveA3 r) {
        float y = CTRL_FIRST_ROW_Y;
        for (EpreuveOraleControle ctrl : r.getEpreuvesDeControle()) {
    ...
    private void ecrireEducationPhysique(PdfContentByte cb, BaseFont font, ReleveA3 r) {
    ...
    private void ecrireEpreuvesFacultatives(PdfContentByte cb, BaseFont font, ReleveA3 r) {
    ...
    private void ecrireTotalDefinitif(PdfContentByte cb, BaseFont font, ReleveA3 r) {
        texte(cb, font, 10, TOTAL_DEFINITIF_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, ReleveA3 r) {
        // ---- 1er groupe ----
