...
import com.officedubac.project.modules.A3.model.Enums.*;
...
import static com.officedubac.project.modules.A3.pdf.RelevNoteA3Coordinates.*;

/**
 * Génère le PDF du relevé A3 en superposant les données saisies sur le
 * gabarit officiel (fond de page), aux coordonnées mesurées dans {@link RelevNoteA3Coordinates}.
 */
@Service
public class RelevNoteA3PdfService {
    ...
    public byte[] genererPdf(RelevNoteA3 releve) {
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF A3", ex);
            }
        }
    }
    ...
    private void ecrireSession(PdfContentByte cb, RelevNoteA3 r) {
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, RelevNoteA3 r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, RelevNoteA3 r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteA3 r) {
        ...
        List<Matiere> matieres = MatieresA3.PREMIER_GROUPE;
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteA3 r) {
        ...
        Map<String, NoteEpreuve> parCode = r.getNotesDeuxiemeGroupe().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresA3.DEUXIEME_GROUPE;
        for (int i = 0; i < matieres.size(); i++) {
            ligneNote(cb, font, parCode.get(matieres.get(i).getCode()), G2_NOTE_CENTER_X, G2_POINTS_CENTER_X, G2_ROW_Y[i]);
        }
    }

    private void ecrireEpreuvesControle(PdfContentByte cb, BaseFont font, RelevNoteA3 r) {
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteA3 r) {
    ...
    private void ecrireTotaux(PdfContentByte cb, BaseFont font, RelevNoteA3 r) {
        texteCentre(cb, font, TAILLE_TOTAUX, G2_POINTS_CENTER_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, RelevNoteA3 r) {
    ...
    private void ecrirePiedDePage(PdfContentByte cb, BaseFont font, RelevNoteA3 r, float lieuX, float jourMoisX, float annee2X, float y) {
