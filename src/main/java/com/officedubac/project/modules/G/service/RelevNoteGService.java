package com.officedubac.project.modules.G.service;
...
import com.officedubac.project.modules.G.model.*;
import com.officedubac.project.modules.G.model.Enums.DecisionJury;
import com.officedubac.project.modules.G.model.Enums.Mention;
import com.officedubac.project.modules.G.repository.RelevNoteGRepository;
 ...
 * selon les règles imprimées sur le formulaire officiel série G.
 */
@Service
public class RelevNoteGService {

    private final RelevNoteGRepository repository;

    public RelevNoteGService(RelevNoteGRepository repository) {
        this.repository = repository;
    }

    public RelevNoteG creer(RelevNoteGSaisieRequest request) {
        RelevNoteG releve = new RelevNoteG();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteG mettreAJour(String id, RelevNoteGSaisieRequest request) {
        RelevNoteG releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteG obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteGResume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteG> page;
    ...
    private RelevNoteGResume versResume(RelevNoteG r) {
    ...
    private void appliquer(RelevNoteG releve, RelevNoteGSaisieRequest request) {
        ...
        for (Matiere matiere : MatieresG.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresG.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteGSaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteGSaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresG.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresG.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresG.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresG.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresG.BAREME_TOTAL_DEFINITIF));
