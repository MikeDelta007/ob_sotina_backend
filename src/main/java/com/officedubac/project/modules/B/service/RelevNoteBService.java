...
import com.officedubac.project.modules.B.repository.RelevNoteBRepository;
 ...
 */
@Service
public class RelevNoteBService {

    private final RelevNoteBRepository repository;

    public RelevNoteBService(RelevNoteBRepository repository) {
        this.repository = repository;
    }

    public RelevNoteB creer(RelevNoteBSaisieRequest request) {
        RelevNoteB releve = new RelevNoteB();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteB mettreAJour(String id, RelevNoteBSaisieRequest request) {
        RelevNoteB releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteB obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteBResume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteB> page;
    ...
    private RelevNoteBResume versResume(RelevNoteB r) {
    ...
    private void appliquer(RelevNoteB releve, RelevNoteBSaisieRequest request) {
        ...
        for (Matiere matiere : MatieresB.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteBSaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteBSaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
