...
import static com.officedubac.project.modules.c2emepartie.pdf.ReleveC2emePartieCoordinates.*;

/**
 * Génère le PDF du relevé C-2emePartie en superposant les données saisies sur le
 * gabarit officiel (fond de page).
 */
@Service
public class ReleveC2emePartiePdfService {

    private static final String TEMPLATE_PATH = "templates/releve-C-1erePartie-template.pdf";
    ...
    public byte[] genererPdf(ReleveC2emePartie releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé C-2emePartie en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF C-2emePartie", ex);
            }
        }
    }
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, ReleveC2emePartie r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, ReleveC2emePartie r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrireEpreuvesEcrites(PdfContentByte cb, BaseFont font, ReleveC2emePartie r) {
        Map<String, NoteEpreuve> parCode = r.getNotesEcrites().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresC2emePartie.EPREUVES_ECRITES;
    ...
    private void ecrireEpreuvesOrales(PdfContentByte cb, BaseFont font, ReleveC2emePartie r) {
        Map<String, NoteEpreuve> parCode = r.getNotesOrales().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresC2emePartie.EPREUVES_ORALES;
        for (int i = 0; i < matieres.size(); i++) {
    ...
    private void ecrireTotalGeneral(PdfContentByte cb, BaseFont font, ReleveC2emePartie r) {
        texte(cb, font, TAILLE_TOTAL_GEN, TOTAL_GENERAL_X, TOTAL_GENERAL_Y, str(r.getTotalGeneral()));
    }

    private void ecrireDecision(PdfContentByte cb, BaseFont font, ReleveC2emePartie r) {
        DecisionJury d = r.getDecision();
        if (d == null) return;
        texte(cb, font, TAILLE_DECISION, DEC_TEXTE_X, DEC_TEXTE_Y, d == DecisionJury.ADMIS ? "ADMIS" : "AJOURNE");
    }

    private void ecrirePiedDePage(PdfContentByte cb, BaseFont font, ReleveC2emePartie r) {
