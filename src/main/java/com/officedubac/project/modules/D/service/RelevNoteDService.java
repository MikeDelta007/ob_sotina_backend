...
import com.officedubac.project.modules.D.repository.RelevNoteDRepository;
 ...
 */
@Service
public class RelevNoteDService {

    private final RelevNoteDRepository repository;

    public RelevNoteDService(RelevNoteDRepository repository) {
        this.repository = repository;
    }

    public RelevNoteD creer(RelevNoteDSaisieRequest request) {
        RelevNoteD releve = new RelevNoteD();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteD mettreAJour(String id, RelevNoteDSaisieRequest request) {
        RelevNoteD releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteD obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteDResume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteD> page;
    ...
    private RelevNoteDResume versResume(RelevNoteD r) {
    ...
    private void appliquer(RelevNoteD releve, RelevNoteDSaisieRequest request) {
        ...
        for (Matiere matiere : MatieresD.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteDSaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteDSaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
