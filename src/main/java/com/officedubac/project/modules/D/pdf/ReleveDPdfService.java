...
import static com.officedubac.project.modules.D.pdf.ReleveDCoordinates.*;

/**
 * Génère le PDF du relevé D en superposant les données saisies sur une
 * copie du formulaire officiel scanné (fond de page), aux coordonnées
 * mesurées dans {@link ReleveDCoordinates}.
 */
@Service
public class ReleveDPdfService {

    private static final String TEMPLATE_PATH = "templates/releve-D-template.pdf";
    private static final DateTimeFormatter DATE_FR = DateTimeFormatter.ofPattern("dd/MM/yyyy");

    public byte[] genererPdf(ReleveD releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé D en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, ReleveD r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, ReleveD r) {
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, ReleveD r) {
        var parCode = r.getNotesPremierGroupe().stream()
                .collect(java.util.stream.Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n));

        ligneNote(cb, font, parCode.get(MatieresD.FRANCAIS_ECRIT.getCode()), G1_FR_ECRIT_Y);
        ligneNote(cb, font, parCode.get(MatieresD.FRANCAIS_ORAL.getCode()),  G1_FR_ORAL_Y);
        ligneNote(cb, font, parCode.get(MatieresD.PHILOSOPHIE.getCode()),    G1_PHILO_Y);
        ligneNote(cb, font, parCode.get(MatieresD.MATHEMATIQUES.getCode()), G1_MATH_Y);
        ligneNote(cb, font, parCode.get(MatieresD.SCIENCES_PHYSIQUES.getCode()), G1_SCPHY_Y);
        ligneNote(cb, font, parCode.get(MatieresD.SCIENCES_NATURELLES.getCode()), G1_SCNAT_Y);
        ligneNote(cb, font, parCode.get(MatieresD.HIST_GEO.getCode()),      G1_HISTGEO_Y);
        ligneNote(cb, font, parCode.get(MatieresD.LV.getCode()),            G1_LV_Y);
    ...
    private void ecrireEpreuvesDeControle(PdfContentByte cb, BaseFont font, ReleveD r) {
    ...
    private void ecrireEducationPhysique(PdfContentByte cb, BaseFont font, ReleveD r) {
    ...
    private void ecrireEpreuvesFacultatives(PdfContentByte cb, BaseFont font, ReleveD r) {
    ...
    private void ecrireTotalDefinitif(PdfContentByte cb, BaseFont font, ReleveD r) {
        texte(cb, font, 10, TOTAL_DEFINITIF_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, ReleveD r) {
