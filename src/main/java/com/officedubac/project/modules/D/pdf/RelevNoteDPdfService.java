...
import com.officedubac.project.modules.D.model.Enums.*;
...
import static com.officedubac.project.modules.D.pdf.RelevNoteDCoordinates.*;

/**
 * Génère le PDF du relevé D en superposant les données saisies sur le
 * gabarit officiel (fond de page), aux coordonnées mesurées dans {@link RelevNoteDCoordinates}.
 */
@Service
public class RelevNoteDPdfService {
    ...
    public byte[] genererPdf(RelevNoteD releve) {
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF D", ex);
            }
        }
    }
    ...
    private void ecrireSession(PdfContentByte cb, RelevNoteD r) {
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, RelevNoteD r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, RelevNoteD r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteD r) {
        ...
        List<Matiere> matieres = MatieresD.PREMIER_GROUPE;
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteD r) {
        ...
        Map<String, NoteEpreuve> parCode = r.getNotesDeuxiemeGroupe().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresD.DEUXIEME_GROUPE;
        for (int i = 0; i < matieres.size(); i++) {
            ligneNote(cb, font, parCode.get(matieres.get(i).getCode()), G2_NOTE_CENTER_X, G2_POINTS_CENTER_X, G2_ROW_Y[i]);
        }
    }

    private void ecrireEpreuvesControle(PdfContentByte cb, BaseFont font, RelevNoteD r) {
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteD r) {
    ...
    private void ecrireTotaux(PdfContentByte cb, BaseFont font, RelevNoteD r) {
        texteCentre(cb, font, TAILLE_TOTAUX, G2_POINTS_CENTER_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, RelevNoteD r) {
    ...
    private void ecrirePiedDePage(PdfContentByte cb, BaseFont font, RelevNoteD r, float lieuX, float jourMoisX, float annee2X, float y) {
