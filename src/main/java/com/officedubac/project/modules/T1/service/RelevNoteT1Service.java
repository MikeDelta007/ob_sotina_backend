package com.officedubac.project.modules.T1.service;
...
import com.officedubac.project.modules.T1.model.*;
import com.officedubac.project.modules.T1.model.Enums.DecisionJury;
import com.officedubac.project.modules.T1.model.Enums.Mention;
import com.officedubac.project.modules.T1.repository.RelevNoteT1Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série T1.
 */
@Service
public class RelevNoteT1Service {

    private final RelevNoteT1Repository repository;

    public RelevNoteT1Service(RelevNoteT1Repository repository) {
        this.repository = repository;
    }

    public RelevNoteT1 creer(RelevNoteT1SaisieRequest request) {
        RelevNoteT1 releve = new RelevNoteT1();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteT1 mettreAJour(String id, RelevNoteT1SaisieRequest request) {
        RelevNoteT1 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteT1 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteT1Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteT1> page;
    ...
    private RelevNoteT1Resume versResume(RelevNoteT1 r) {
    ...
    private void appliquer(RelevNoteT1 releve, RelevNoteT1SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresT1.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresT1.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteT1SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteT1SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresT1.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresT1.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresT1.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresT1.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresT1.BAREME_TOTAL_DEFINITIF));
