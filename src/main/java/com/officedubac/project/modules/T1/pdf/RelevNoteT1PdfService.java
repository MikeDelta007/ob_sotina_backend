...
import com.officedubac.project.modules.T1.model.Enums.*;
...
import static com.officedubac.project.modules.T1.pdf.RelevNoteT1Coordinates.*;

/**
 * Génère le PDF du relevé T1 en superposant les données saisies sur le
 * gabarit officiel (fond de page), aux coordonnées mesurées dans {@link RelevNoteT1Coordinates}.
 */
@Service
public class RelevNoteT1PdfService {

    private static final String TEMPLATE_PATH = "templates/releve-T1-template.pdf";
    ...
    public byte[] genererPdf(RelevNoteT1 releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé T1 en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF T1", ex);
            }
        }
    }
    ...
    private void ecrireSession(PdfContentByte cb, RelevNoteT1 r) {
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, RelevNoteT1 r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, RelevNoteT1 r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteT1 r) {
        ...
        List<Matiere> matieres = MatieresT1.PREMIER_GROUPE;
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteT1 r) {
        ...
        Map<String, NoteEpreuve> parCode = r.getNotesDeuxiemeGroupe().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresT1.DEUXIEME_GROUPE;
        for (int i = 0; i < matieres.size(); i++) {
            ligneNote(cb, font, parCode.get(matieres.get(i).getCode()), G2_NOTE_CENTER_X, G2_POINTS_CENTER_X, G2_ROW_Y[i]);
        }
    }

    private void ecrireEpreuvesControle(PdfContentByte cb, BaseFont font, RelevNoteT1 r) {
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteT1 r) {
    ...
    private void ecrireTotaux(PdfContentByte cb, BaseFont font, RelevNoteT1 r) {
        texteCentre(cb, font, TAILLE_TOTAUX, G2_POINTS_CENTER_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, RelevNoteT1 r) {
    ...
    private void ecrirePiedDePage(PdfContentByte cb, BaseFont font, RelevNoteT1 r, float lieuX, float jourMoisX, float annee2X, float y) {
