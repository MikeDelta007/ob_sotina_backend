package com.officedubac.project.modules.S2.service;
...
import com.officedubac.project.modules.S2.model.Enums.DecisionJury;
import com.officedubac.project.modules.S2.model.Enums.Mention;
import com.officedubac.project.modules.S2.repository.RelevNoteS2Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série S2.
 */
@Service
public class RelevNoteS2Service {

    private final RelevNoteS2Repository repository;

    public RelevNoteS2Service(RelevNoteS2Repository repository) {
        this.repository = repository;
    }

    public RelevNoteS2 creer(RelevNoteS2SaisieRequest request) {
        RelevNoteS2 releve = new RelevNoteS2();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteS2 mettreAJour(String id, RelevNoteS2SaisieRequest request) {
        RelevNoteS2 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteS2 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteS2Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteS2> page;
    ...
    private RelevNoteS2Resume versResume(RelevNoteS2 r) {
    ...
    private void appliquer(RelevNoteS2 releve, RelevNoteS2SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresS2.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresS2.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteS2SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteS2SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresS2.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresS2.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresS2.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresS2.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresS2.BAREME_TOTAL_DEFINITIF));
