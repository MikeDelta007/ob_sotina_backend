package com.officedubac.project.modules.L2.service;
...
import com.officedubac.project.modules.L2.model.*;
import com.officedubac.project.modules.L2.model.Enums.DecisionJury;
import com.officedubac.project.modules.L2.model.Enums.Mention;
import com.officedubac.project.modules.L2.repository.RelevNoteL2Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série L2.
 */
@Service
public class RelevNoteL2Service {

    private final RelevNoteL2Repository repository;

    public RelevNoteL2Service(RelevNoteL2Repository repository) {
        this.repository = repository;
    }

    public RelevNoteL2 creer(RelevNoteL2SaisieRequest request) {
        RelevNoteL2 releve = new RelevNoteL2();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteL2 mettreAJour(String id, RelevNoteL2SaisieRequest request) {
        RelevNoteL2 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteL2 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteL2Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteL2> page;
    ...
    private RelevNoteL2Resume versResume(RelevNoteL2 r) {
    ...
    private void appliquer(RelevNoteL2 releve, RelevNoteL2SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresL2.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresL2.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteL2SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteL2SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresL2.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresL2.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresL2.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresL2.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresL2.BAREME_TOTAL_DEFINITIF));
