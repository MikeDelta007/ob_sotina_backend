...
import com.officedubac.project.modules.L2.model.Enums.*;
...
import static com.officedubac.project.modules.L2.pdf.RelevNoteL2Coordinates.*;

/**
 * Génère le PDF du relevé L2 en superposant les données saisies sur le
 * gabarit officiel (fond de page), aux coordonnées mesurées dans {@link RelevNoteL2Coordinates}.
 */
@Service
public class RelevNoteL2PdfService {

    private static final String TEMPLATE_PATH = "templates/releve-L2-template.pdf";
    ...
    public byte[] genererPdf(RelevNoteL2 releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé L2 en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF L2", ex);
            }
        }
    }
    ...
    private void ecrireSession(PdfContentByte cb, RelevNoteL2 r) {
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, RelevNoteL2 r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, RelevNoteL2 r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteL2 r) {
        ...
        List<Matiere> matieres = MatieresL2.PREMIER_GROUPE;
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteL2 r) {
        ...
        Map<String, NoteEpreuve> parCode = r.getNotesDeuxiemeGroupe().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresL2.DEUXIEME_GROUPE;
        for (int i = 0; i < matieres.size(); i++) {
            ligneNote(cb, font, parCode.get(matieres.get(i).getCode()), G2_NOTE_CENTER_X, G2_POINTS_CENTER_X, G2_ROW_Y[i]);
        }
    }

    private void ecrireEpreuvesControle(PdfContentByte cb, BaseFont font, RelevNoteL2 r) {
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteL2 r) {
    ...
    private void ecrireTotaux(PdfContentByte cb, BaseFont font, RelevNoteL2 r) {
        texteCentre(cb, font, TAILLE_TOTAUX, G2_POINTS_CENTER_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, RelevNoteL2 r) {
    ...
    private void ecrirePiedDePage(PdfContentByte cb, BaseFont font, RelevNoteL2 r, float lieuX, float jourMoisX, float annee2X, float y) {
