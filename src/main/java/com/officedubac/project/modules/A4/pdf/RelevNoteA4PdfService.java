...
import com.officedubac.project.modules.A4.model.Enums.*;
...
import static com.officedubac.project.modules.A4.pdf.RelevNoteA4Coordinates.*;

/**
 * Génère le PDF du relevé A4 en superposant les données saisies sur le
 * gabarit officiel (fond de page), aux coordonnées mesurées dans {@link RelevNoteA4Coordinates}.
 */
@Service
public class RelevNoteA4PdfService {
    ...
    public byte[] genererPdf(RelevNoteA4 releve) {
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF A4", ex);
            }
        }
    }
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, RelevNoteA4 r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteA4 r) {
        ...
        List<Matiere> matieres = MatieresA4.PREMIER_GROUPE;
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteA4 r) {
        ...
        Map<String, NoteEpreuve> parCode = r.getNotesDeuxiemeGroupe().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresA4.DEUXIEME_GROUPE;
        for (int i = 0; i < matieres.size(); i++) {
            ligneNote(cb, font, parCode.get(matieres.get(i).getCode()), G2_NOTE_CENTER_X, G2_POINTS_CENTER_X, G2_ROW_Y[i]);
        }
    }

    private void ecrireEpreuvesControle(PdfContentByte cb, BaseFont font, RelevNoteA4 r) {
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteA4 r) {
    ...
    private void ecrireTotaux(PdfContentByte cb, BaseFont font, RelevNoteA4 r) {
