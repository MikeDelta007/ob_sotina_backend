...
import com.officedubac.project.modules.A4.repository.RelevNoteA4Repository;
 ...
 */
@Service
public class RelevNoteA4Service {

    private final RelevNoteA4Repository repository;

    public RelevNoteA4Service(RelevNoteA4Repository repository) {
        this.repository = repository;
    }

    public RelevNoteA4 creer(RelevNoteA4SaisieRequest request) {
        RelevNoteA4 releve = new RelevNoteA4();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteA4 mettreAJour(String id, RelevNoteA4SaisieRequest request) {
        RelevNoteA4 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteA4 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteA4Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteA4> page;
    ...
    private RelevNoteA4Resume versResume(RelevNoteA4 r) {
    ...
    private void appliquer(RelevNoteA4 releve, RelevNoteA4SaisieRequest request) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteA4SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteA4SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
