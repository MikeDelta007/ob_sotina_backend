...
import com.officedubac.project.modules.F1.model.Enums.TypeFacultative;
import com.officedubac.project.modules.F1.repository.ReleveF1Repository;
 ...
 *
 * Particularité F1 (comme A3/B/D/E) : le barème complet (580) est atteint
 * dès le 1er groupe. Le 2eme groupe ne fait qu'ajuster ce total (contrôle,
 * éducation physique, facultatives).
 *
 * Comme B, D et E, l'épreuve de contrôle a sa propre colonne "Cœff" : les
 * points au contrôle = nouvelle note × coefficient.
 */
@Service
public class ReleveF1Service {

    private final ReleveF1Repository repository;

    public ReleveF1Service(ReleveF1Repository repository) {
        this.repository = repository;
    }

    public ReleveF1 creer(ReleveF1SaisieRequest request) {
        ReleveF1 releve = new ReleveF1();
        appliquer(releve, request);
        return repository.save(releve);
    }

    public ReleveF1 mettreAJour(String id, ReleveF1SaisieRequest request) {
        ReleveF1 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public ReleveF1 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    private void appliquer(ReleveF1 releve, ReleveF1SaisieRequest request) {
        ...
        if (request.getEpreuvesDeControle() != null) {
            for (ReleveF1SaisieRequest.EpreuveDeControleSaisie s : request.getEpreuvesDeControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (ReleveF1SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
