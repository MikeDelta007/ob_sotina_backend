package com.officedubac.project.modules.F1.service;
...
import com.officedubac.project.modules.F1.repository.RelevNoteF1Repository;
 ...
 * selon les règles imprimées sur le formulaire officiel série F1.
 */
@Service
public class RelevNoteF1Service {

    private final RelevNoteF1Repository repository;

    public RelevNoteF1Service(RelevNoteF1Repository repository) {
        this.repository = repository;
    }

    public RelevNoteF1 creer(RelevNoteF1SaisieRequest request) {
        RelevNoteF1 releve = new RelevNoteF1();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteF1 mettreAJour(String id, RelevNoteF1SaisieRequest request) {
        RelevNoteF1 releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteF1 obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteF1Resume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteF1> page;
    ...
    private RelevNoteF1Resume versResume(RelevNoteF1 r) {
    ...
    private void appliquer(RelevNoteF1 releve, RelevNoteF1SaisieRequest request) {
        ...
        for (Matiere matiere : MatieresF1.PREMIER_GROUPE) {
        ...
        for (Matiere matiere : MatieresF1.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteF1SaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteF1SaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
        ...
        Mention mention1erGroupe = mention(totalPremierGroupe, MatieresF1.BAREME_PREMIER_GROUPE);
        releve.setMentionPremierGroupe(mention1erGroupe);
        releve.setDecisionPremierGroupe(decision(totalPremierGroupe, MatieresF1.BAREME_PREMIER_GROUPE));

        int totalDefinitifAvecFacultatifs = totalDefinitif;
        Mention mentionAvantBonus = mention(totalDefinitif, MatieresF1.BAREME_TOTAL_DEFINITIF);
        ...
        releve.setMentionDeuxiemeGroupe(mention(totalDefinitifAvecFacultatifs, MatieresF1.BAREME_TOTAL_DEFINITIF));
        releve.setDecisionDeuxiemeGroupe(decision(totalDefinitifAvecFacultatifs, MatieresF1.BAREME_TOTAL_DEFINITIF));
