...
import static com.officedubac.project.modules.A3_DeuxiemePartie.pdf.ReleveA3DeuxiemePartieCoordinates.*;

/**
 * Génère le PDF du relevé A3 2ème Partie en superposant les données saisies
 * sur une copie du formulaire officiel scanné (fond de page), aux
 * coordonnées mesurées dans {@link ReleveA3DeuxiemePartieCoordinates}.
 */
@Service
public class ReleveA3DeuxiemePartiePdfService {
    ...
    public byte[] genererPdf(ReleveA3DeuxiemePartie releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé A3 2ème Partie en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private void ecrireEpreuvesEcrites(PdfContentByte cb, BaseFont font, ReleveA3DeuxiemePartie r) {
        ...
        ligneNote(cb, font, parCode.get(MatieresA3DeuxiemePartie.PHILOSOPHIE.getCode()), EC_NOTE_X, EC_POINTS_X, EC_PHILO_Y);
        ligneNote(cb, font, parCode.get(MatieresA3DeuxiemePartie.LV1_ECRIT.getCode()),   EC_NOTE_X, EC_POINTS_X, EC_LV1_Y);
        ligneNote(cb, font, parCode.get(MatieresA3DeuxiemePartie.HIST_GEO.getCode()),    EC_NOTE_X, EC_POINTS_X, EC_HISTGEO_Y);
        ligneNote(cb, font, parCode.get(MatieresA3DeuxiemePartie.LV2_ECRIT.getCode()),   EC_NOTE_X, EC_POINTS_X, EC_LV2_Y);
    ...
    private void ecrireEpreuvesOrales(PdfContentByte cb, BaseFont font, ReleveA3DeuxiemePartie r) {
        ...
        ligneNote(cb, font, parCode.get(MatieresA3DeuxiemePartie.LV1_ORAL.getCode()),      OR_NOTE_X, OR_POINTS_X, OR_LV1_Y);
        ligneNote(cb, font, parCode.get(MatieresA3DeuxiemePartie.MATHEMATIQUES.getCode()), OR_NOTE_X, OR_POINTS_X, OR_MATHS_Y);
