...
import com.officedubac.project.modules.E.repository.RelevNoteERepository;
 ...
 */
@Service
public class RelevNoteEService {

    private final RelevNoteERepository repository;

    public RelevNoteEService(RelevNoteERepository repository) {
        this.repository = repository;
    }

    public RelevNoteE creer(RelevNoteESaisieRequest request) {
        RelevNoteE releve = new RelevNoteE();
        releve.setCreatedAt(Instant.now());
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteE mettreAJour(String id, RelevNoteESaisieRequest request) {
        RelevNoteE releve = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
        appliquer(releve, request);
        return repository.save(releve);
    }

    public RelevNoteE obtenir(String id) {
        return repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Relevé introuvable : " + id));
    }

    public Page<RelevNoteEResume> lister(String numeroTable, Integer annee, Pageable pageable) {
        boolean hasNumeroTable = numeroTable != null && !numeroTable.isBlank();
        boolean hasAnnee = annee != null;

        Page<RelevNoteE> page;
    ...
    private RelevNoteEResume versResume(RelevNoteE r) {
    ...
    private void appliquer(RelevNoteE releve, RelevNoteESaisieRequest request) {
        ...
        for (Matiere matiere : MatieresE.DEUXIEME_GROUPE) {
        ...
        if (request.getEpreuvesOralesControle() != null) {
            for (RelevNoteESaisieRequest.EpreuveOraleControleSaisie s : request.getEpreuvesOralesControle()) {
        ...
        if (request.getEpreuvesFacultatives() != null) {
            for (RelevNoteESaisieRequest.EpreuveFacultativeSaisie s : request.getEpreuvesFacultatives()) {
