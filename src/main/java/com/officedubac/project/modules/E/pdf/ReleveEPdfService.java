...
import static com.officedubac.project.modules.E.pdf.ReleveECoordinates.*;

/**
 * Génère le PDF du relevé E en superposant les données saisies sur une
 * copie du formulaire officiel scanné (fond de page), aux coordonnées
 * mesurées dans {@link ReleveECoordinates}.
 */
@Service
public class ReleveEPdfService {

    private static final String TEMPLATE_PATH = "templates/releve-E-template.pdf";
    private static final DateTimeFormatter DATE_FR = DateTimeFormatter.ofPattern("dd/MM/yyyy");

    public byte[] genererPdf(ReleveE releve) {
        try (InputStream templateStream = new ClassPathResource(TEMPLATE_PATH).getInputStream();
            ...
            throw new IllegalStateException("Erreur lors de la génération du relevé E en PDF: " + e.getMessage(), e);
        }
    }
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, ReleveE r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, ReleveE r) {
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, ReleveE r) {
        var parCode = r.getNotesPremierGroupe().stream()
                .collect(java.util.stream.Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n));

        ligneNote(cb, font, parCode.get(MatieresE.FRANCAIS_ECRIT.getCode()), G1_FR_ECRIT_Y);
        ligneNote(cb, font, parCode.get(MatieresE.FRANCAIS_ORAL.getCode()),  G1_FR_ORAL_Y);
        ligneNote(cb, font, parCode.get(MatieresE.PHILOSOPHIE.getCode()),    G1_PHILO_Y);
        ligneNote(cb, font, parCode.get(MatieresE.MATHEMATIQUES.getCode()), G1_MATH_Y);
        ligneNote(cb, font, parCode.get(MatieresE.SCIENCES_PHYSIQUES.getCode()), G1_SCPHY_Y);
        ligneNote(cb, font, parCode.get(MatieresE.CONSTRUCTION_MECANIQUE.getCode()), G1_CONSTRMECA_Y);
        ligneNote(cb, font, parCode.get(MatieresE.ANALYSE_FABRICATION_TECHNO.getCode()), G1_ANFABR_Y);
        ligneNote(cb, font, parCode.get(MatieresE.TECHNIQUE_PRATIQUE.getCode()), G1_TECHPRAT_Y);
        ligneNote(cb, font, parCode.get(MatieresE.LV.getCode()),            G1_LV_Y);
    ...
    private void ecrireEpreuvesDeControle(PdfContentByte cb, BaseFont font, ReleveE r) {
    ...
    private void ecrireEducationPhysique(PdfContentByte cb, BaseFont font, ReleveE r) {
    ...
    private void ecrireEpreuvesFacultatives(PdfContentByte cb, BaseFont font, ReleveE r) {
    ...
    private void ecrireTotalDefinitif(PdfContentByte cb, BaseFont font, ReleveE r) {
        texte(cb, font, 10, TOTAL_DEFINITIF_X, TOTAL_DEFINITIF_Y, str(r.getTotalDefinitif()));
    }

    private void ecrireDecisions(PdfContentByte cb, BaseFont font, ReleveE r) {
