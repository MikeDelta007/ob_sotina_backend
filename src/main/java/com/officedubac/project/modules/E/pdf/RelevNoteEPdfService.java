...
import com.officedubac.project.modules.E.model.Enums.*;
...
import static com.officedubac.project.modules.E.pdf.RelevNoteECoordinates.*;

/**
 * Génère le PDF du relevé E en superposant les données saisies sur le
 * gabarit officiel (fond de page), aux coordonnées mesurées dans {@link RelevNoteECoordinates}.
 */
@Service
public class RelevNoteEPdfService {
    ...
    public byte[] genererPdf(RelevNoteE releve) {
    ...
    private BaseFont chargerPolice() {
        try (InputStream in = new ClassPathResource(POLICE_PATH).getInputStream()) {
            ...
            try {
                ...
                throw new IllegalStateException("Impossible de charger une police pour le PDF E", ex);
            }
        }
    }
    ...
    private void ecrireSession(PdfContentByte cb, RelevNoteE r) {
    ...
    private void ecrireEnTete(PdfContentByte cb, BaseFont font, RelevNoteE r) {
    ...
    private void ecrireCandidat(PdfContentByte cb, BaseFont font, RelevNoteE r) {
        Candidat c = r.getCandidat();
        if (c == null) return;
    ...
    private void ecrirePremierGroupe(PdfContentByte cb, BaseFont font, RelevNoteE r) {
        ...
        List<Matiere> matieres = MatieresE.PREMIER_GROUPE;
    ...
    private void ecrireDeuxiemeGroupe(PdfContentByte cb, BaseFont font, RelevNoteE r) {
        ...
        Map<String, NoteEpreuve> parCode = r.getNotesDeuxiemeGroupe().stream()
                .collect(Collectors.toMap(NoteEpreuve::getMatiereCode, n -> n, (a, b) -> a));
        List<Matiere> matieres = MatieresE.DEUXIEME_GROUPE;
        for (int i = 0; i < matieres.size(); i++) {
            ligneNote(cb, font, parCode.get(matieres.get(i).getCode()), G2_NOTE_CENTER_X, G2_POINTS_CENTER_X, G2_ROW_Y[i]);
        }
    }

    private void ecrireEpreuvesControle(PdfContentByte cb, BaseFont font, RelevNoteE r) {
    ...
    private void ecrireEpreuvesFacultativesEtEducPhysique(PdfContentByte cb, BaseFont font, RelevNoteE r) {
    ...
    private void ecrireTotaux(PdfContentByte cb, BaseFont font, RelevNoteE r) {
